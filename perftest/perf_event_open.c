...
init(void)
{
    ...
    fd_perf = syscall(__NR_perf_event_open, &attr, 0, -1, -1, 0);
}
...
{
    long long result = 0;
    if (read(fd_perf, &result, sizeof(result)) < sizeof(result))
        return 0;
    return result;
}
...
int main(int ac, char **av)
{
    ...
    printf("[PMU_DEMO] %s: beginning loop\n", av[0]);
    time_start = cpucycles();
    sum = loop(a, b, len);
    time_end = cpucycles();
    printf("[PMU_DEMO] %s: done. sum = %d; time delta = %llu\n", av[0], sum, time_end - time_start);
