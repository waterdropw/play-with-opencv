...
if [[ ${TARGET_OS} == "Android" ]]; then
    ...
    cmake ${CMAKE_CONFIG} -D ANDROID_ABI="armeabi-v7a with NEON" -S ${SRC_DIR} -B ${BUILD_DIR}
    cmake --build ${BUILD_DIR} --config Release --target install -- -j${CPU_CORES}

    BUILD_DIR=${BUILD_ROOT}/aarch64
    cmake ${CMAKE_CONFIG} -D ANDROID_ABI=arm64-v8a -S ${SRC_DIR} -B ${BUILD_DIR}
    cmake --build ${BUILD_DIR} --config Release --target install -- -j${CPU_CORES}
else
