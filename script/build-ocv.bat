...
set INSTALL_ROOT=.\out\opencv\distrib\%TARGET_OS%
...
set ANDROID_BUILD_32=%BUILD_ROOT%\armeabi-v7a
set ANDROID_BUILD_64=%BUILD_ROOT%\arm64-v8a
...
if %TARGET_OS% == Android (
    ...
    cmake -G "Visual Studio 16 2019" -A x64 ^
        -D CMAKE_BUILD_TYPE=Release ^
        -D CMAKE_INSTALL_PREFIX=%INSTALL_DIR%\x64-Release ^
        -D CMAKE_POSITION_INDEPENDENT_CODE=ON ^
        -D OPENCV_EXTRA_MODULES_PATH=%EXTRA_MODULE_PATH% ^
        -D ENABLE_CXX11=ON ^
        -D BUILD_SHARED_LIBS=ON ^
